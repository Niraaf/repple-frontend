const ExerciseListItem = ({ ex, onAdd, isAdded }) => {
  return (
    <div className="flex items-center justify-between p-4 bg-gray-50 border border-gray-200 rounded-xl hover:shadow-sm transition">

      {/* Info */}
      <div className="flex flex-col">
        <p className="font-semibold text-sm text-gray-800">{ex.name}</p>
        <div className="flex flex-wrap gap-1 mt-1">
          {ex.muscle_groups.slice(0, 2).map(group => (
